...
let inputSchematic = String.raw``;
let inputTestSchematic1 = String.raw`467..114..
...*......
..35..633.
......#...
617*......
...
const inputTestSchematic2 = String.raw`........
.24..4..
......*.`;

const inputAssignmentSchematic = String.raw`....401.............425.......323......791......697...............963............................................420........................
...*..................................%......#.....*....290.........................492.............656...@953.....................+830.....
...
inputSchematic = inputAssignmentSchematic;
const inputAsStringArr = [...inputSchematic];


const lines: string[][] = [];
let line: string[] = [];
inputAsStringArr.forEach(c => {
    if (c === "\n") {
        lines.push(line);
        line = [];
        return;
...
lines.push(line);
const ROW_WIDTH = lines[0].length;
const ROWS_COUNT = lines.length;
...
const numbersFound: NumberFound[] = [];
const isNumber = (x: any): x is number => !Number.isNaN(Number(x));
const makeNumberFoundWithDefaults = (rowIdx: number): NumberFound => ({
    row: rowIdx,
    startCol: 0,
    endCol: 0,
    value: 0
...
lines.forEach((row, rowIdx) => {
    let numberFound = makeNumberFoundWithDefaults(rowIdx);
    let numberFoundValue = "";
    row.forEach((col, colIdx) => {
        if(isNumber(col)) {
            const prev = row[colIdx - 1];
            const next = row[colIdx + 1];
            if (!isNumber(prev)) {
                numberFound.startCol = colIdx;
            }

            numberFoundValue += `${col}`;
            if (!isNumber(next)) {
                numberFound.value = Number(numberFoundValue);
                numberFound.endCol = colIdx;

                // Add finding to results
                numbersFound.push(numberFound);
                
                // Clear temp values
                numberFound = makeNumberFoundWithDefaults(rowIdx);
                numberFoundValue = "";
...
//log(numbersFound);
//log(numbersFound.filter(n => n.row === 126));
//log(numbersFound.filter(n => n.row === ROWS_COUNT - 1));

function findNeighbours(numberFound: NumberFound): [number, number][] {
    let neighbours: [number, number][] = [];
    
    const { row, startCol, endCol } = numberFound;
    
    if (startCol > 0) {
        neighbours.push([row, startCol - 1]);
    }
    if (endCol < ROW_WIDTH) {
        neighbours.push([row, endCol + 1]);
    }

    let verticalSearchLength = `${numberFound.value}`.length + 2;
    let verticalSearchStartCol = startCol - 1;
    if (startCol == 0) {
        verticalSearchLength--;
        verticalSearchStartCol++;
    }
    if (endCol === ROW_WIDTH - 1) {
        verticalSearchLength--;
    }

    if (row > 0) {
        neighbours = neighbours.concat(new Array(verticalSearchLength)
            .fill([row - 1, verticalSearchStartCol])
            .map(([row, col], idx) => ([row, col + idx]))
        );
    }
    if (row !== ROWS_COUNT - 1) {
        neighbours = neighbours.concat(new Array(verticalSearchLength)
...
//console.assert(JSON.stringify(findNeighbours(numbersFound.find(n => n.row === 0 && n.startCol === 4)!)) === JSON.stringify([[0, 3], [0, 7], [1, 3], [1, 4], [1, 5], [1, 6], [1, 7]]));

//console.log(findNeighbours(numbersFound.find(n => n.row === 0 && n.startCol === 4)!));
//console.log(findNeighbours(numbersFound.find(n => n.row === 2 && n.startCol === 2)!));
//console.log(findNeighbours(numbersFound.find(n => n.row === ROWS_COUNT - 1 && n.startCol === 5)!));

const symbolsFound = inputAsStringArr.filter(c => /[^(\d|\.|\n)]/.test(c));
//const symbols = [...new Set(symbolsFound).values()];
const symbols = new Set(symbolsFound);
log([...symbols.values()]);

const symbolLocations: Record<number, Record<number, string>> = {};
lines.forEach((row, rowIdx) => {
    row.forEach((col, colIdx) => {
        if (symbols.has(col)) {
            if (symbolLocations[rowIdx] === undefined) {
                symbolLocations[rowIdx] = {};
            }
            symbolLocations[rowIdx][colIdx] = col;
        }
    });
});
